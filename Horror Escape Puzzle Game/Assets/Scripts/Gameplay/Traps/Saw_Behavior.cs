...
public class Saw_Behavior : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        path.positionCount = waypoints.Length;
        if (behavior == Behavior.Translate) path.positionCount ++;
    ...
    private void DrawPath()
    {
        ...
        if(behavior == Behavior.Translate)
        {
        ...
        {
            path.SetPosition(0, anchor.position);
            path.SetPosition(1, this.gameObject.transform.position);
        }
    ...
    //TODO What if the saw orbits attached to an anchor?
